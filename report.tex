...
\begin{tikzpicture}[
    box/.style={draw, rounded corners, align=center, minimum width=3.1cm, minimum height=0.9cm, font=\footnotesize},
    arrow/.style={-Latex, thick},
    darrow/.style={-Latex, thick, dashed}
]
% Layer 1 (top): APs
\node[box] (ap1) at (0, 5.5) {AP1 (Real Router)\\SSID Michal\_2.4\\BSSID d6:35:1d:9f:89:fc};
\node[box] (ap2) at (11, 5.5) {AP2 (Tenda USB)\\Same SSID + WPA2\\BSSID c8:3a:35:c2:e0:b7};

% Layer 2: client phone (high, centered)
\node[box] (client) at (5.5, 3.6) {Client Phone\\32:0a:af:e3:81:b2};

% Layer 3 (bottom): VM and monitor
\node[box] (vm) at (0, 1) {Ubuntu VM (VirtualBox)\\lab\_cli.py wizard};
\node[box] (mon) at (11, 1) {Monitor (EDUP USB)\\wlxe84e06aed7c8\\passive capture};

% Wi-Fi: side routes through open space (y=3.6)
\draw[arrow] (client.west) -- (2.1, 3.6) -- (ap1.east)
    node[pos=0.3, above=1mm] {Wi-Fi};
\draw[arrow] (client.east) -- (9.4, 3.6) -- (ap2.west)
    node[pos=0.3, above=1mm] {Wi-Fi};

% Sniff AP2: straight vertical on the right
\draw[darrow] (mon.north) -- node[right=2mm, pos=0.45] {sniff} (ap2.south);

% Sniff AP1: left margin channel (x=-2), avoids VM/client boxes
\draw[darrow] (mon.west) -- (8.2, 2.4) -- (-2, 2.4) -- (-2, 5.5) -- (ap1.west)
    node[pos=0.42, left=1mm] {sniff};

% Control plane along bottom (y=0.15), below VM/monitor
\draw[arrow] (vm.east) -- node[above=1mm, pos=0.5] {capture} (mon.west);
\draw[arrow] (vm.south) -- (5.5, 0.15) -- node[below=1mm, pos=0.4] {hostapd + dnsmasq} (12.6, 0.15) -- (ap2.south east);
\end{tikzpicture}%
}
